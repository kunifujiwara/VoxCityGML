...
- `export_obj.py:1048` (fallback per-category OBJ export) also reaches the
  building branch, so exported building voxels adopt the new behaviour too —
  intended, keeps exports consistent with the main grid.
